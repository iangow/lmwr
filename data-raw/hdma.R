...
hdma <-
  ...
  mutate(across(c(emp, clines, suff, assets, school, s57, s48, s39,
         ...
         pr = s33/s50,
         coap = s16==4,
         lwage_coap = if_else(s31c > 0, log(s31c), NA_real_),
         lwage_coap2 = if_else(coap, lwage_coap, NA_real_),
         male_coap = s16 == 1) %>%
  select(seq, deny, black, wage, chist, mhist, phist, emp,
         clines, suff, assets, school,
         s20, s24a, s27a, s39, s48, s53, s55, s56, s57,
         chval, married, lr, pr, coap,
         lwage_coap, lwage_coap2, male_coap)
